...
TabItemDashGraph <- R6Class(
  ...
  public = list(
    ...
    initialize = function(
      ...
      if(dataInput=="csvInput"){
        self$generateSidebarChoices(columnNames = sidebarShownLabels,
                                    columnTypes, columnOptions, dataSubClasses)
      }
    },
    ...
    makeSidebarPanel = function(chartType){
      if(chartType=="googleChart") {
          if(self$dataInput == "userInput"){
              sidebarPanelList = self$makeTextInput()
          } else {
